...
def test_lambda_syntax():
    ...
    assert __ == func_invoker(lambda_increment, 10)
    # this is the common usage of lambda
    assert __ == func_invoker(lambda x : x+1, 10)
    assert __ == func_invoker(nested_func, 10)

    #write your first lambda below !
    assert 12 == func_invoker(__, 10)


def test_lambda_has_access_to_locals():
    value = 10
    assert __ == func_invoker(lambda x: x+value, 10)
...
def test_lambda_local_binding():
    ...
    assert __ == adder_1(100)
    assert __ == adder_2(100)
